...
behave, taken from the flvs CMS: the sidebar shell and page header, instant navigation into
...
The developer's instruction: groundwork must copy flvs's design and components, and "it's
very important that the skill files remember these things". The registry already carries
some flvs parts (table card, status pill, filter chip, pagination) but nothing tells a project
how to assemble a screen, so each one re-decides — the brian2 admin built this week used top
...
- [ ] `skills/app-ui/SKILL.md` exists with frontmatter and sections for shell, page header,
      ...
      stats/panels, value pickers, motion and copy; every numeric value in it appears in flvs
...
- Reworking brian2's admin to the doctrine (that project's own feature)
