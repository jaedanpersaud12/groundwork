...
not class choices), not a port of the upstream `imprint` skill it started from.
...
through several sessions drifts the way real project UI does without something like `imprint`
...
- **Not a port of the upstream `imprint`.** That skill's whole extraction list (`bg-`,
...
- **A 7th lifecycle skill, not a groundwork-only one.** Installed by `kit init` into every
  ...
  matching the upstream setup, where `imprint` came from the same skill source as
  `architect`/`recover`/`remember`.

## Done when

- [x] `skills/imprint/SKILL.md` exists, adapted per the decision above (verified by reading
      it back against the upstream one and confirming the extraction list actually
...
- [x] `kit init` installs 7 skills, not 6 — `packages/kit/src/assets.ts`'s `readSkills()`
      ...
      *"Installed 7 skills" against a fresh scratch project*
- [x] `kit doctor`'s `"kickoff"` bucket checks for the 7th skill — same automatic pickup,
      confirmed by running `kit doctor` against that same fresh project — *15/15
      checks pass*
- [x] `apps/registry`'s `kit.ts` build-time consistency check (every skill in `skills/` must
      ...
      *the build genuinely failed first, naming `imprint`, before the fix*
- [x] `ui-registry.md` is back in `templates/next16-insforge/`, seeded with the file's
      ...
      exists — *`grep -c "^### " ...` → `0`, no entry headings at all*
- [x] A real run of `/imprint` against a genuinely composed (non-registry-primitive) piece
      ...
      against something in the throwaway project from 09, or a fresh equivalent — *composed `@ja3dan/empty-state` + `@ja3dan/button`, the resulting entry
...
- [x] `bun run check` passes — *confirmed on CI (a fresh machine), this branch: `check`
      ...
      settles it — the local run was environmental, not a real failure.*

## Out of scope

- **Audit mode** (`/imprint audit`). Real and useful in the upstream version — scanning an
