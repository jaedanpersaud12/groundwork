...
idea" to a repo whose `context/` is as specific as a mature, hand-written one — without assuming they use
...
that actually makes an existing app cheap to work on — 3,774 lines of context that say exactly
...
- **Three generation stages, one copy step.** Each stage's output is the next stage's
  ...
  single prompt was the original design and it was wrong: roughly half of a real
  project's context either isn't knowable at kickoff or has to be read out of installed packages.
...
- **No `ui-registry.md` in the template.** One is genuinely useful when the project has
...
- **Proven by re-deriving a real app's context and diffing**, not by reading the output and nodding.

## Done when

- [x] `prompts/01-interview.md`, run against a real app answering as its developer would,
      produces a `project-overview.md` that a line-by-line diff against the real file shows
      to contain: every page, the data-ownership rules, every analytics
      event, a features-out-of-scope list of at least 15 items, and success criteria —
      *Tested harder than the spec asked: the simulated developer
      gave a deliberately vague first out-of-scope answer, and the prompt's rule pushed
      back and reached 18 items.*
- [x] `prompts/02-architecture.md`, given the app's **real** `project-overview.md`,
      ...
      and an invariants list — diffed against the real file — *5 data-flow diagrams produced; quality held through the schema section and past it,
      settling the stage-2 length risk without a split.*
- [x] `prompts/03-build-plan.md`, given the app's real overview and architecture, produces
      numbered features in phases, each with the UI/Logic split — *21 features, 3 phases;
      independently reproduced the real plan's one-page-many-features pattern (one page's UI,
      save, extraction and export as four separate features) without having seen the real
      file.*
- [x] The three run **end to end** from a cold start on the same app, and the compounding drift
      is written down — where stage 3's output degrades because stage 1's was imperfect —
      *One real, traceable drift found: stage 1's Data Ownership
...
- [x] Every interview stage refuses to accept a vague answer: asked "what's out of scope?",
      a reply of "not much" produces another question, not a shrug. Demonstrated during the run.
- [x] `templates/next16-insforge/` contains the five copied files, with every
      app-specific fact removed — verified by grepping the template for the source app's
      name and third-party services and finding only stack-level mentions — *every
      `InsForge` hit is a stack-level pattern.*
- [x] A reader following `/docs` alone can get from nothing to the three prompts and knows
      what each produces before running it — *`/docs/kickoff`
...
- **More than one preset.** `next16-insforge` comes from one existing app. A second preset is only
