...
| `apps/registry` | Next 16 preview/docs app; `registry.json` + `registry/groundwork/**` are the component sources; builds `public/r/*.json` | shadcn registry `@ja3dan`, deployed on Vercel |
...
Planned, not built: `packages/kit` (CLI: check, doctor, sync, link), `templates/`, `presets/`, `apps/site` (marketing site, design TBD). See `context/build-plan.md`.
